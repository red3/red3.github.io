...
2.针对单一文件build phases-compile sources中定位相应的文件 -fno-objc-arc
...
从上例可以看到，用autorelease，可以使得对象存在，但自己不持有对象。autorelease提供这样的功能，使对象在超出指定的生存范围时能够自动并正确地释放。

这个正确的释放时机，通过自动释放池（autoreleasePool）来完成，注册在这个池子里的对象，会在自动释放池结束的时候去释放池子里的资源。



推荐书籍iOS与OS X多线程和内存管理
...
前面提到过，ARC时代之所以方便，就是因为编译器替我们在合适的地方主动加上了release 代码，所以程序员无需再次键入release。
